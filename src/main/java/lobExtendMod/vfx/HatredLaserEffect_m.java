 ...
 * @author hoykj
 */
public class HatredLaserEffect_m extends AbstractGameEffect {
    private float sX;
    private float sY;
    private static TextureAtlas.AtlasRegion img;
    private boolean end;

    public HatredLaserEffect_m(Skeleton skeleton){
        Bone bone;
        bone = skeleton.findBone("Magic_Razor");
        if (img == null) {
            img = ImageMaster.vfxAtlas.findRegion("combat/laserThin");
        }
        this.color = Color.WHITE.cpy();
        this.color.a = 0;
        this.sX = skeleton.getX() + bone.getWorldX() + 50;
        this.sY = skeleton.getY() + bone.getWorldY() - 200;
    ...
    public void render(SpriteBatch sb) {
        ...
        sb.draw(img, this.sX, this.sY - 20.0F * Settings.scale, 0.0F, img.packedHeight / 2.0F,
